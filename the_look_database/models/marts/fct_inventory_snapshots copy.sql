{{
    config(
        ...
        unique_id='inventory_snapshot_sk',
...
main as (
    select 
        ...
        (
            sum(case when ad.dt < dp.dt then ad.counts_added else 0 end) 
            - sum(case when sl.dt < dp.dt then sl.counts_removed_via_orders else 0 end) 
            + sum(case when rt.dt < dp.dt then rt.counts_returned else 0 end) 
        ...
        sum(case when ad.dt = dp.dt then ad.counts_added else 0 end) as units_received_today,
        sum(case when sl.dt = dp.dt then sl.counts_removed_via_orders else 0 end) as units_sold_today,
        sum(case when rt.dt = dp.dt then rt.counts_returned else 0 end) as units_returned_today,

        (
            sum(case when ad.dt < dp.dt then ad.cost_added else 0 end) 
            - coalesce((sum(case when ad.dt < dp.dt then ad.cost_added else 0 end)/nullif(sum(case when ad.dt < dp.dt then ad.counts_added else 0 end), 0)), 0) * sum(case when sl.dt < dp.dt then sl.counts_removed_via_orders else 0 end) 
            + coalesce((sum(case when ad.dt < dp.dt then ad.cost_added else 0 end)/nullif(sum(case when ad.dt < dp.dt then ad.counts_added else 0 end), 0)), 0) * sum(case when rt.dt < dp.dt then rt.counts_returned else 0 end)
        ...
        (
            sum(case when ad.dt < dp.dt then ad.value_added else 0 end)
            - coalesce((sum(case when ad.dt < dp.dt then ad.value_added else 0 end)/nullif(sum(case when ad.dt < dp.dt then ad.counts_added else 0 end), 0)), 0) * sum(case when sl.dt < dp.dt then sl.counts_removed_via_orders else 0 end) 
            + coalesce((sum(case when ad.dt < dp.dt then ad.value_added else 0 end)/nullif(sum(case when ad.dt < dp.dt then ad.counts_added else 0 end), 0)), 0) * sum(case when rt.dt < dp.dt then rt.counts_returned else 0 end)
        ...
        max(case when ad.dt <= dp.dt and ad.counts_added > 0 then dp.dt end) as last_restocked_at
...
select sum(total_inventory_in_stock_at_open), sum(total_inventory_in_stock_at_close) from final